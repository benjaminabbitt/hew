...
func (l *lowerer) emit(path Path, nodes []*mirrorEntry, surface Surface, q quals) error {
	...
	for i, e := range nodes {
		...
		case marginRemove:
			...
			if leadsRemovedMember(nodes, i, paired) {
				continue // the member's removal takes its leading comment
			}
...
// leadsRemovedMember reports whether the removed comment at nodes[i] is the
// LEADING comment of a member the same container removes (§8.2: "immediately
// preceding, with no blank line"). It is, when the very next entry is an
...
// comments the patch happened to list, not the container's. A comment above
// a REPLACED member is not absorbed: that member survives the apply.
func leadsRemovedMember(nodes []*mirrorEntry, i int, paired []int) bool {
	if nodes[i].kind != mComment {
		return false
	}
	next := i + 1
	if next >= len(nodes) {
	...
	n := nodes[next]
	return n.node() && n.margin == marginRemove && n.kind != mComment && paired[next] < 0
}
...
func (l *lowerer) freeAssert(path Path, e *mirrorEntry) error {
	...
	switch a.verb {
	case verbExhaustive:
		// `? exhaustive` -> test with exhaustive: true (§9.1 step 3), paired
		// with the count of children this hunk lists at that level so an
		// applier can evaluate it without re-reading the patch.
		n := 0
		...
		t.Path, t.Count, t.Exhaustive = path, &n, true
...
func matchScalar(n *yaml.Node, line int) (Scalar, error) {
	...
	return Scalar{Kind: ScalarString, Text: n.Value, Quoted: needsMatchQuote(n.Value)}, nil
}

// needsMatchQuote reports a string that must be quoted inside a `=value` path
// segment: one whose bare spelling would read back as another kind (a number,
// a boolean, null), one the path grammar would split or mis-tokenize, or one
// carrying whitespace — §4.2's own `id="a b"` example.
func needsMatchQuote(s string) bool {
	return needsQuote(s) || strings.ContainsAny(s, " \t")
}
