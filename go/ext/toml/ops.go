...
func (r *run) evalValue(t hew.Transform) error {
	...
	e.TargetLine = lineOf(r.d.src, rf.node.start)
	return e
}
...
func (r *run) planAdd(t hew.Transform) ([]edit, error) {
	...
	if he == nil {
		...
		// hold an array still means the whole node, not its container:
		// `! upsert` replaces regardless of current value (§7.7), and
		// appending into the old array on a replace corrupts the target.
