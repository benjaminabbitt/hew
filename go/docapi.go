...
func (s *Sel) place(sibling string, before bool) *Sel {
	...
	if strings.HasPrefix(sibling, "/") {
		...
		seg, err := parseSegment(sibling, false)
		if err != nil {
...
// commitFS is the §10.5 commit, held behind a package variable because A.8's
// hewfs owns this half of the contract (it is being built on p5/cli-rulings).
// Doc.Write is written against the property — atomic temp-and-rename, no
// backup file — so hewfs takes it over by assignment rather than by a change
// to any caller here.
var commitFS = writeAtomic

func writeAtomic(fsys afero.Fs, name string, data []byte) error {
	perm := fs.FileMode(0o644)
	if fi, err := fsys.Stat(name); err == nil {
		perm = fi.Mode().Perm()
	}
	tmp, err := afero.TempFile(fsys, filepath.Dir(name), "."+filepath.Base(name)+".hew")
	if err != nil {
		return writeErr(name, err)
	}
	staged := tmp.Name()
	...
	// Best effort: a backend with no permission model reports one error here
	// and is no less written for it.
	_ = fsys.Chmod(staged, perm)
	if err := fsys.Rename(staged, name); err != nil {
		_ = fsys.Remove(staged)
		return writeErr(name, err)
	}
	return nil
