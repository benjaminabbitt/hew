...
func ValueOf(x any) (v Value, err error) {
	// yaml.Node.Encode panics on an unencodable type rather than returning.
	defer func() {
		if r := recover(); r != nil {
			v, err = Value{}, fmt.Errorf("cannot represent %T as a hew value: %v", x, r)
		}
	}()
	var n yaml.Node
