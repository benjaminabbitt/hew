...
func TestDiscoverMissingDeclaredFiles(t *testing.T) {
	tests := []struct {
		...
		{
			// yaml/pragma-idempotent-file declares parse to pin a property of
			// the patch TEXT without pinning the lowered list: parse succeeding
			// is the whole assertion, so a missing transforms fixture is legal.
			name: "ok parse seam without transforms fixture is legal",
			files: map[string]string{
		...
		{
			name: "diff seam missing only new.*",
			files: map[string]string{
				"case.yaml": manifestYAML("json/c", "[diff]", "ok"),
				"old.json":  "o", "expected.hew": "e",
			},
			want: []string{"declares seam diff but has no old.*/new.*/expected.hew"},
		},
		{
			name: "diff seam missing only old.*",
			files: map[string]string{
				"case.yaml": manifestYAML("json/c", "[diff]", "ok"),
				"new.json":  "n", "expected.hew": "e",
			},
			want: []string{"declares seam diff but has no old.*/new.*/expected.hew"},
		},
		{
			name: "diff seam missing only expected.hew",
			files: map[string]string{
				"case.yaml": manifestYAML("json/c", "[diff]", "ok"),
				"old.json":  "o", "new.json": "n",
			},
			want: []string{"declares seam diff but has no old.*/new.*/expected.hew"},
		},
		{
...
// TestDiscoverUnreadableFamilyDoesNotStopTheWalk: one broken family is
// reported, and every later family is still discovered — a corpus error must
// not silently shrink the suite.
func TestDiscoverUnreadableFamilyDoesNotStopTheWalk(t *testing.T) {
	if os.Geteuid() == 0 {
		t.Skip("root ignores directory permissions")
	}
	root := t.TempDir()
	broken := filepath.Join(root, "aaa") // sorts before the good family
	if err := os.MkdirAll(broken, 0o755); err != nil {
		t.Fatal(err)
	}
	writeCase(t, root, "zzz/good-case", happyFiles("zzz/good-case"))
	...
	cases, errs := Discover(root)
	requireErrContaining(t, errs, "family aaa:")
	if len(cases) != 1 || cases[0].Rel != "zzz/good-case" {
		t.Fatalf("families after a broken one must still be walked, got %v", cases)
	}
}
