...
fixture: |
  git init .
targets_unchanged: [a.json, b.json]
expected_targets:
  a.json: expected-a.json
  b.json: expected-b.json
no_files_created: ["*.hewr", ".hew/*"]
expected_record: record.json
record_digest_fields: [target_digest, patch_digest]
`)
	...
	if !reflect.DeepEqual(m.TargetsUnchanged, []string{"a.json", "b.json"}) {
		t.Errorf("TargetsUnchanged = %v", m.TargetsUnchanged)
	}
	wantTargets := map[string]string{"a.json": "expected-a.json", "b.json": "expected-b.json"}
	if !reflect.DeepEqual(m.ExpectedTargets, wantTargets) {
		t.Errorf("ExpectedTargets = %v, want %v", m.ExpectedTargets, wantTargets)
	}
	if !reflect.DeepEqual(m.NoFilesCreated, []string{"*.hewr", ".hew/*"}) {
		t.Errorf("NoFilesCreated = %v", m.NoFilesCreated)
	}
	if m.ExpectedRecord != "record.json" {
		t.Errorf("ExpectedRecord = %q", m.ExpectedRecord)
	}
	if !reflect.DeepEqual(m.RecordDigestFields, []string{"target_digest", "patch_digest"}) {
		t.Errorf("RecordDigestFields = %v", m.RecordDigestFields)
	}
}
...
// TestSortedSeamsIsStableWithinARank: seams that share an execution rank —
// which includes any seam the order table does not know — keep their declared
// order, so the report reads the way the manifest was written.
func TestSortedSeamsIsStableWithinARank(t *testing.T) {
	m := Manifest{Seams: []Seam{SeamCLI, "zzz", SeamParse, "yyy"}}
	got := m.SortedSeams()
	want := []Seam{"zzz", SeamParse, "yyy", SeamCLI}
	if !reflect.DeepEqual(got, want) {
		t.Errorf("SortedSeams() = %v, want %v", got, want)
	}
}
