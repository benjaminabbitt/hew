...
func TestComputeCoverageEdgeCases(t *testing.T) {
	...
	t.Run("an unknown ref does not stop the rest of the case's ops", func(t *testing.T) {
		catalog := []CatalogOp{{ID: "OP-01", Tier: "v0"}, {ID: "OP-02", Tier: "v0"}}
		cov := ComputeCoverage(catalog, []*Case{opsCase("json/a", "OP-99", "OP-01", "OP-02")})
		if cov.CasesPerOp["OP-01"] != 1 || cov.CasesPerOp["OP-02"] != 1 {
			t.Errorf("CasesPerOp = %v; refs after an unknown one must still count", cov.CasesPerOp)
		}
		if len(cov.UncoveredV0) != 0 {
			t.Errorf("UncoveredV0 = %v, want none", cov.UncoveredV0)
		}
