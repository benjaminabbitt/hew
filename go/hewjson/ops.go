...
func (d *doc) planInsert(target string, path, before, after hew.Path, value hew.Value, text string, onConflict hew.OnConflict, idempotent bool, line int) (*edit, error) {
	if existing, err := d.resolveFull(target, path, line); err == nil {
		if existing.kind == jArr {
			return d.insertArrayElement(existing, before, after, text, line)
		}
	...
	return d.insertObjectMember(parent, last.Name, before, after, text, line)
}
...
func (d *doc) insertObjectMember(obj *jNode, key string, before, after hew.Path, valueText string, line int) (*edit, error) {
...
func (d *doc) insertArrayElement(arr *jNode, before, after hew.Path, valueText string, line int) (*edit, error) {
	afterIdx, beforeIdx := -1, -1
	if !after.IsZero() {
		n, rerr := d.resolveFull("", after, line)
		if rerr != nil {
