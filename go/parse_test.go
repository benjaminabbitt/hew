...
func lowered(t *testing.T, src string) string {
	...
	files := p.Files()
	if len(files) != 1 {
		t.Fatalf("Files() = %d sections, want 1", len(files))
	}
	b, err := MarshalTransforms(files[0].Transforms())
	if err != nil {
...
// directive beats the file pragma beats the strict default.
...
// TestParseDocument pins the §2 file grammar and the Appendix A.2 accessors.
func TestParseDocument(t *testing.T) {
	...
	if got := files[0].Target(); got != "my config.yaml" {
		t.Errorf("Target() = %q, want %q (a quoted path with a space, §2.2)", got, "my config.yaml")
	}
	if got := files[0].Format(); got != FormatYAML {
		t.Errorf("Format() = %q, want the preamble default %q", got, FormatYAML)
	}
	if got := files[1].Format(); got != FormatJSON {
		t.Errorf("Format() = %q, want the format= attribute %q", got, FormatJSON)
	}
	if got := files[0].Line(); got != 5 {
		t.Errorf("Line() = %d, want 5", got)
	}
	if got := files[0].Transforms().Target; got != "my config.yaml" {
		t.Errorf("Transforms().Target = %q", got)
	}
	hunks := files[0].Hunks()
	if len(hunks) != 1 {
	...
	h := hunks[0]
	if !h.Anchor().Equal(MustParsePath("/server")) {
		t.Errorf("Anchor() = %s, want /server", h.Anchor())
	}
	if h.Line() != 7 {
		t.Errorf("Line() = %d, want 7", h.Line())
	}
	if !h.AssertOnly() {
		t.Error("AssertOnly() = false; a hunk with no +/- lines is assert-only (§7.4)")
	}
	if got := h.Transforms(); len(got) != 1 || got[0].Op != OpTest || got[0].PatchLine != 8 {
		t.Errorf("Transforms() = %+v, want one test from line 8", got)
	}
	if files[1].Hunks()[0].AssertOnly() {
		t.Error("AssertOnly() = true for a hunk with a `-` line")
	}
}
