...
// (§8.3), and TOML (§8.4) appliers are bound, as are Resolve (§9.2) and
// --ops/--record (§9.7). Remaining: the HCL applier, the differ (P4), and
...
var skipRules = []harness.SkipRule{
	...
	// TOML applies for real now (§8.4's surface duality); only its diff seam
	// still waits on the differ.
	{Case: "toml/*", Seam: "diff", Reason: "M11: differ not yet implemented (P4)"},

	// HCL parses and renders for real (§8.5 attributes/blocks/labels, §7.2
	// ordinals); what it has yet is an applier, a differ, or a CLI backend.
